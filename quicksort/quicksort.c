...
int dbl_cmp(const void *a, const void *b) {
    // TODO
    double *a2 = (double *)a, *b2 = (double *)b;
...
static void swap(void *a, void *b, size_t size) {
    ...
    char *a2 = (char *)a, *b2 = (char *)b, holder;

    for (int i = 0; i < size; i++)
        holder = *(a2 + i), *(a2 + i) = *(b2 + i), *(b2 + i) = holder;
}
 ...
 * pointer arithmetic.
 */
// elem_sz is how large each element is
// function pointer for last element
static int lomuto(void *array, int left, int right, size_t elem_sz, int (*comp)(const void *, const void *)) {
    char *arr = array, *p = (arr + (left * elem_sz));
    int s = left;

    for (int i = left + 1; i <= right; i++)
        if (comp((arr + (i * elem_sz)), p) < 0)
            s++, swap((arr + (s * elem_sz)), (arr + (i * elem_sz)), elem_sz);
