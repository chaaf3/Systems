 ...
 * Date        : 6/7/21
 * Description : head implementation.
...
int main(int argc, char *argv[]) {
    ...
    if (n_value) {
        if (!is_integer(n_value) || !get_integer(n_value, &line_count) ||
            line_count < 0) {
            printf(
                "Warning: Invalid line count '%s' supplied. "
    ...
    int num_bytes = (line_count-1) * sizeof(char *);
    int sz = read(fd, c, num_bytes);
    c[sz] = '\0';

    int trash_file = open("/dev/tty", O_WRONLY);
